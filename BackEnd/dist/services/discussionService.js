...
const hasDiscussionKeyword = (content) => {
...
const getThreadAnchorMessageId = (messages) => {
...
class DiscussionService {
    constructor(discussionStateRepository) {
        this.discussionStateRepository = discussionStateRepository;
    }
    async setDiscussionStatus(discussionId, status) {
        return this.discussionStateRepository.setStatus(discussionId, status);
    }
    async detect(messagesByChannel, channelNameMap, channelIds) {
        ...
        const referenceTime = Math.max(...channels.flatMap((entry) => entry.messages.map((message) => message.timestamp.getTime())));
