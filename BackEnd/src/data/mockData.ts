...
const getAllMessageContexts = (): MessageContext[] => {
  const channelNameMap = buildChannelNameMap();
  const allMessageRecords = [messages, groupMessages];

  return allMessageRecords.flatMap((record) =>
    Object.entries(record).flatMap(([channelId, channelMessages]) =>
...
const filterByChannelIds = <T extends { channelId: string }>(
  items: T[],
  channelIds?: string[],
) => {
...
export const getUnansweredQuestions = (channelIds?: string[]): DetectedQuestion[] => {
  ...
  Object.values(byChannel).forEach((channelEntries) => {
    const ordered = [...channelEntries].sort(
      (left, right) => left.message.timestamp.getTime() - right.message.timestamp.getTime(),
    );
...
const getDiscussionStatus = (lastActivity: Date, referenceTime: number): DetectedDiscussion['status'] => {
...
export const getDetectedDiscussions = (channelIds?: string[]): DetectedDiscussion[] => {
  const contexts = filterByChannelIds(getAllMessageContexts(), channelIds);
  const byChannel = contexts.reduce((acc, entry) => {
    if (!acc[entry.channelId]) {
      acc[entry.channelId] = [];
  ...
  const discussions = Object.entries(byChannel)
    .map(([channelId, channelEntries]) => {
      ...
      } satisfies DetectedDiscussion;
    })
    .filter((discussion): discussion is DetectedDiscussion => Boolean(discussion));
